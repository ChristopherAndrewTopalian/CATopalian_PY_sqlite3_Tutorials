# ⚠️ HAZARD ZONE: Hardcoded String Insertions

**DO NOT USE THIS METHOD FOR GUI APPLICATIONS OR USER INPUT.**

### The "Technically Correct" Trap
The code below is technically valid Pure SQL. If you are writing a private, static script where *you* (the programmer) are the only one typing the data, this code will execute perfectly.

### Why it is Dangerous
This method builds a dangerous habit. If you take this exact logic and connect it to a graphical interface (like a PySide6 text box) using Python string formatting, your database will crash the moment a user types an apostrophe (e.g., `O'Connor`). Worse, it leaves your application entirely open to **SQL Injection**, allowing malicious users to delete your tables.

**The Industry Standard:** Always use Parameterized Queries (the `?` method) taught in the main `CATopalian_PY_SQLite3_Tutorials` folder. This legacy example is kept here strictly to demonstrate what a vulnerable data pattern looks like.
